...
export const client = new Gsoc2Client({
...
export const getAwsCloudWatchLog = async () => {
  const logGroupName =
    (await client.getSecret("AWS_CLOUDWATCH_LOG_GROUP_NAME")).secretValue || "gsoc2-log-stream";
...
export const getSmtpFromName = async () =>
  (await client.getSecret("SMTP_FROM_NAME")).secretValue || "Gsoc2";
...
export const getIsGsoc2Cloud = async () =>
  (await client.getSecret("GSOC2_CLOUD")).secretValue === "true";
...
export const getLicenseServerUrl = async () =>
  (await client.getSecret("LICENSE_SERVER_URL")).secretValue || "https://portal.gsoc2.com";
