...
const syncSecretsNetlify = async ({
  ...
  Object.keys(secrets).map((key) => {
    if (!(key in res)) {
      // case: Gsoc2 secret does not exist in Netlify -> create secret
      newSecrets.push({
        key,
        values: [
          {
      ...
      // case: Gsoc2 secret exists in Netlify
      ...
      if (integration.targetEnvironment in contexts) {
        ...
        if (secrets[key] !== contexts[integration.targetEnvironment].value) {
          // case: Gsoc2 and Netlify secret values are different
  ...
  Object.keys(res).map((key) => {
    ...
    if (!(key in secrets)) {
      // case: Netlify secret does not exist in Gsoc2
...
const syncSecretsCloudflarePages = async ({
  ...
  if (getSecretsRes) {
    for await (const key of Object.keys(getSecretsRes)) {
      if (!(key in secrets)) {
        // case: secret does not exist in gsoc2
