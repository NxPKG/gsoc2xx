...
import { client, getEncryptionKey, getIsGsoc2Cloud, getRootEncryptionKey } from "../../config";
import {
  ADMIN,
    ...
    try {
      ...
      for (const [key, value] of roleMap.entries()) {
        ...
        const role = new Role({
          ...
          workspace: workspaceId,
          description:
            "This role was auto generated by Gsoc2 in effort to migrate your project members to our new permission system",
          isOrgRole: false,
...
export const migrationAssignSuperadmin = async () => {
  ...
  if (await getIsGsoc2Cloud()) {
    await updateServerConfig({ initialized: true });
    logger.info("Backfill: Gsoc2 Cloud(initialized)");
    return;
  }
  ...
  if (users.length) {
    ...
    logger.info("Backfill: Migrated first gsoc2 user to super admin");
  }
};
