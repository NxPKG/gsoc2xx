  ...
  () => {
    ...
    return (
      <div className="container mx-auto bg-bunker-800 text-white w-full h-full max-w-7xl px-6">
        ...
        <div className="text-xl font-semibold text-gray-200 mb-2">Gsoc2 Rotation Providers</div>
          ...
          {!isRotationProviderLoading &&
            secretRotationProviders?.providers.map((provider) => (
              <div
                ...
                key={`gsoc2-rotation-provider-${provider.name}`}
                tabIndex={0}
                role="button"
                onKeyDown={(evt) => {
        ...
        >
          ...
          >
            {t("integrations.why-gsoc2-needs-access")}
          </ModalContent>
        </Modal>
        <DeleteActionModal
        ...
        <UpgradePlanModal
          ...
          text="You can add secret rotation if you switch to Gsoc2's Team plan."
        />
      </div>
    );
  },
