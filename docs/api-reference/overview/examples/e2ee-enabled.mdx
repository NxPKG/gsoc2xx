...
Using Gsoc2's API to read/write secrets with E2EE enabled allows you to create, update, and retrieve secrets
...
const BASE_URL = 'https://app.gsoc2.com';
...
const getSecrets = async () => {
    ...
    // 3. Decrypt the (encrypted) project key with the key from your Gsoc2 Token
...
const createSecrets = async () => {
    ...
    // 2. Decrypt the (encrypted) project key with the key from your Gsoc2 Token
    const projectKey = decrypt({
        ciphertext: serviceTokenData.encryptedKey,
        iv: serviceTokenData.iv,
        tag: serviceTokenData.tag,
	...
	// 4. Send (encrypted) secret to Gsoc2
    await axios.post(
...
const updateSecrets = async () => {
	...
	// 4. Send (encrypted) updated secret to Gsoc2
    await axios.patch(
        `${BASE_URL}/api/v3/secrets/${secretKey}`,
        {
            workspaceId: serviceTokenData.workspace,
...
<Info>
  If using an `API_KEY` to authenticate with the Gsoc2 API, then you should include it in the `X_API_KEY` header.
</Info>
