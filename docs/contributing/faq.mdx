---
title: "FAQ"
description: "Frequently Asked Questions about contributing to Gsoc2"
---

Frequently asked questions about contributing to Gsoc2 can be found on this page. 
    ...
    for mirror in $ALPINE_MIRRORS; do \
        ...
        for i in $(seq $retries_left); do \
            ...
            if apk add --no-cache bash curl git && \
               curl -1sLf 'https://dl.cloudsmith.io/public/gsoc2/gsoc2-cli/setup.alpine.sh' | bash && \
               apk add --no-cache gsoc2=0.8.1; then \
                break; \
            fi; \
            sleep 10; \
        done; \
