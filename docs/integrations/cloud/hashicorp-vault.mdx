...
description: "How to sync secrets from Gsoc2 to HashiCorp Vault"
---

<Note>
    Gsoc2 connects to Vault via the AppRole auth method. 
    
    Currently, each Gsoc2 project can only point and sync secrets to one Vault cluster / namespace
...
In Secrets, enable a KV Secrets Engine at a path for Gsoc2 to sync secrets to; we'll use the path `kv`.
...
Now in Policies, create a new ACL policy scoped to the path(s) you wish Gsoc2 to be able to sync secrets to.

We'll call the policy `test` and have it grant access to the `dev` path in the KV Secrets Engine where we will be syncing secrets to from Gsoc2.
...
We now create a `gsoc2` role with the generated token's time-to-live (TTL) set to 1 hour and can be renewed for up to 4 hours from the time of its creation.
...
```console
vault write auth/approle/role/gsoc2 token_policies="test" token_ttl=1h token_max_ttl=4h
```
...
Finally, we need to generate a **RoleID** and **SecretID** (like a username and password) that Gsoc2 can use
...
```console
vault read auth/approle/role/gsoc2/role-id
```
...
3. Generate a new SecretID of the `gsoc2` role.

```console
vault write -force auth/approle/role/gsoc2/secret-id
```
...
Great. We're now ready to connect Gsoc2 to Vault!

## Enter your Vault instance and authentication details

Back in Gsoc2, press on the HashiCorp Vault tile and input your Vault instance and `gsoc2` role RoleID and SecretID.
...
- Vault RoleID: The RoleID previously created for the `gsoc2` role.
- Vault SecretID: The SecretID previously created for the `gsoc2` role.

## Start integration

Select which Gsoc2 environment secrets you want to sync to Vault.
