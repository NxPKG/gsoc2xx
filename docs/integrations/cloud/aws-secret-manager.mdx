...
description: "How to sync secrets from Gsoc2 to AWS Secrets Manager"
---
...
For better security, here's a custom policy containing the minimum permissions required by Gsoc2 to sync secrets to AWS Secrets Manager for the IAM user that you can use:
...
## Authorize Gsoc2 for AWS Secrets Manager
...
Select which Gsoc2 environment secrets you want to sync to which AWS Secrets Manager region and under which secret name. Then, press create integration to start syncing secrets to AWS Secrets Manager.

![integration create](../../images/integrations/aws/integrations-aws-secret-manager-create.png)

<Info>
  Gsoc2 currently syncs environment variables to AWS Secrets Manager as
