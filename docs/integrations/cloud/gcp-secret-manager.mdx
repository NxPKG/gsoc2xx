...
description: "How to sync secrets from Gsoc2 to GCP Secret Manager"
---
...
In the **Connection** tab, select which Gsoc2 environment secrets you want to sync to which GCP secret manager project. Lastly, press create integration to start syncing secrets to GCP secret manager.
...
- Label in GCP Secret Manager: If selected, every secret will be labeled in GCP Secret Manager (e.g. as `managed-by:gsoc2`); labels can be customized.

Setting a secret prefix, suffix, or enabling the labeling option ensures that existing secrets in GCP Secret Manager are not overwritten during the sync. As part of this process, Gsoc2 abstains from mutating any secrets in GCP Secret Manager without the specified prefix, suffix, or attached label.
...
<Warning>
    Using Gsoc2 to sync secrets to GCP Secret Manager requires that you enable
...
In the **Connection** tab, select which Gsoc2 environment secrets you want to sync to the GCP secret manager project. Lastly, press create integration to start syncing secrets to GCP secret manager.
...
<Warning>
  ...
  Using the GCP Secret Manager integration (via the OAuth2 method) on a self-hosted instance of Gsoc2 requires configuring an OAuth2 application in GCP
  ...
  Navigate to your project API & Services > Credentials to create a new OAuth2 application.
   ...
   Back in your Gsoc2 instance, add two new environment variables for the credentials of your GCP OAuth2 application:
  ...
  Once added, restart your Gsoc2 instance and use the GCP Secret Manager integration.
