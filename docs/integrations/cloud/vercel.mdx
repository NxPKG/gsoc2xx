---
title: "Vercel"
description: "How to sync secrets from Gsoc2 to Vercel"
---
...
## Authorize Gsoc2 for Vercel

Press on the Vercel tile and grant Gsoc2 access to your Vercel account.
...
Select which Gsoc2 environment secrets you want to sync to which Vercel app and environment. Lastly, press create integration to start syncing secrets to Vercel.
...
<Info>
  Gsoc2 syncs every envar to Vercel with type `encrypted` unless an existing
  envar with the same name in Vercel exists with a different type. Note that
  Gsoc2 will not be able to update Vercel envars with type `sensitive` since
...
<Warning>
  ...
  Using the Vercel integration on a self-hosted instance of Gsoc2 requires configuring an integration in Vercel.
  ...
  Navigate to Integrations > Integration Console to create a new integration.
   ...
   ![integrations Vercel config](../../images/integrations/vercel/integrations-vercel-config-new-app.png) 
    ...
    Create the application. As part of the form, set a **URL Slug** to a unique slug like `gsoc2-your-domain` and keep it handy. Also, set **Redirect URL** to `https://your-domain.com/integrations/vercel/oauth2/callback`. Lastly,
   ...
   ## Add your Vercel integration credentials and information to Gsoc2
   ...
   Back in your Gsoc2 instance, add three new environment variables for the credentials of your Vercel integration.
  ...
  Once added, restart your Gsoc2 instance and use the Vercel integration.
