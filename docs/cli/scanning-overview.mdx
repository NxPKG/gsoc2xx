...
Building upon its core functionality of fetching and injecting secrets into your applications, Gsoc2 now takes a significant step forward in bolstering your code security.
...
<Tabs>
  <Tab title="Scanning files, directories and or git history">
	...
	The `gsoc2 scan` command serves to scan repositories, directories, and files. It's compatible with both individual developer machines and Continuous Integration (CI) environments.

	When you run `gsoc2 scan` on a Git repository, Gsoc2 will parses the output of a `git log -p` command. This command generates [patches](https://stackoverflow.com/questions/8279602/what-is-a-patch-in-git-version-control) that Gsoc2 uses to identify secrets in your code.
	...
	For instance, to instruct Gsoc2 to scan a specific range of commits, use the following command: `gsoc2 scan --log-opts="--all commitA..commitB"`. For more details, refer to the [Git log documentation](https://git-scm.com/docs/git-log).
  ...
  <Tab title="Scanning uncommitted files ">
	```bash
	gsoc2 scan git-changes

	# Display the full secret findings
	gsoc2 git-changes --verbose
	```

	Scanning for secrets before you commit your changes is great way to prevent leaks. Gsoc2 makes this easy with the sub command `git-changes`.
	...
	When `git-changes` is run on a Git repository, Gsoc2 parses the output from a `git diff` command.

	To scan changes in commits that have been staged via `git add`, you can add the `--staged` flag to the sub command. This flag is particularly useful when using Gsoc2 CLI as a pre-commit tool.
...
To disable this hook after installing it, run the command `git config --bool hooks.gsoc2-scan false`
...
```bash
gsoc2 scan git-changes --staged --verbose
```
...
A baseline allows Gsoc2 to ignore any old findings that are already present in the baseline findings. You can create a gsoc2 scan report by running `gsoc2 scan` with the `--report-path` flag.

To create a Gsoc2 scan report and save it in a file called leaks-report.json, use the following command:

```
gsoc2 scan --report-path leaks-report.json
```

Once a baseline is created, you can apply it when running the `gsoc2 scan` command again. Use the following command:

```
gsoc2 scan --baseline-path leaks-report.json --report-path findings.json
```
...
# https://raw.githubusercontent.com/Gsoc2/gsoc2/main/cli/config/gsoc2-scan.toml
useDefault = true
# or you can supply a path to a configuration. Path is relative to where gsoc2 cli
...
If you're intentionally committing a test secret that `gsoc2 scan` might flag, you can instruct Gsoc2 to overlook that secret with the methods listed below.

### gsoc2-scan:ignore

To ignore a secret contained in line of code, simply add `gsoc2-scan:ignore ` at the end of the line as comment in the given programming.

```js example.js
function helloWorld() {
    console.log("8dyfuiRyq=vVc3RRr_edRk-fK__JItpZ"); // gsoc2-scan:ignore
}
```

### .gsoc2ignore
An alternative method to exclude specific findings involves creating a .gsoc2ignore file at your repository's root.
You can then add the fingerprints of the findings you wish to exclude. The Gsoc2 scan report provides a unique Fingerprint for each secret found.
By incorporating these Fingerprints into the .gsoc2ignore file, Gsoc2 will skip the corresponding secret findings in subsequent scans.
