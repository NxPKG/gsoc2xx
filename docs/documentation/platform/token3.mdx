---
title: "Service token"
description: "Gsoc2 service tokens allows you to programmatically interact with Gsoc2"
---

Service tokens are authentication credentials that services can use to access designated endpoints in the Gsoc2 API to manage project resources like secrets.
Each service token can be provisioned scoped access to select environment(s) and path(s) within them.

## Service Tokens

Gsoc2 currently offers Service Token V3 and Service Token; you can manage both types of tokens in Project Settings > Service Tokens.
...
<Note>
  Currently, the Service Token V3 authentication method can only be used with the latest [Node SDK](https://github.com/Gsoc2/gsoc2-node) and [Python SDK](https://github.com/Gsoc2/gsoc2-python).
...
`serviceToken` is a JWT token prefixed with `stv3`. The token provides access to the Gsoc2 API and the public-private key
...
before it is an access token. When authenticating with the Gsoc2 API, it is important to send in only the access token portion
of the token.
...
Also, note that Gsoc2 supports [glob patterns](https://www.malikbrowne.com/blog/a-beginners-guide-glob-patterns/) when defining access scopes to path(s).
...
<Warning>
  ...
  If you’re using Gsoc2 Cloud, then it is available under the Pro Tier. If you’re self-hosting Gsoc2, then you should contact team@gsoc2.com to purchase an enterprise license to use it.
</Warning>
...
<AccordionGroup>
<Accordion title="Why is the Gsoc2 API rejecting my service token?">
