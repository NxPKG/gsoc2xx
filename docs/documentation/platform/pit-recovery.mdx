...
description: "How to rollback secrets and configs to any commit with Gsoc2."
---

<Info>
  Point-in-Time Recovery is a paid feature.
   
  If you're using Gsoc2 Cloud, then it is available under the **Team Tier**. If you're self-hosting Gsoc2,
  then you should contact team@gsoc2.com to purchase an enterprise license to use it.
</Info>

Gsoc2's point-in-time recovery feature allows secrets to be rolled back to any point in time for any given [folder](./folder).
Under the hood, snapshots, capturing the state of the folder, get taken after any mutation an item within that folder.

## Snapshots

Similar to Git, a commit (aka snapshot) in Gsoc2 is the state of your project's secrets at a specific point in time scoped to 
