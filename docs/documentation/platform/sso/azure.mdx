---
title: "Azure SAML"
description: "Configure Azure SAML for Gsoc2 SSO"
---

<Info>
   Azure SAML SSO feature is a paid feature.
   
   If you're using Gsoc2 Cloud, then it is available under the **Pro Tier**. If you're self-hosting Gsoc2,
   then you should contact team@gsoc2.com to purchase an enterprise license to use it.
</Info>

1. In Gsoc2, head over to your organization Settings > Authentication > SAML SSO Configuration and select **Set up SAML SSO**.
...
2. On the next screen, press the **+ Create your own application** button.
   Give the application a unique name like Gsoc2; choose the "Integrate any other application you don't find in the gallery (Non-gallery)"
...
<Note>
  If you're self-hosting Gsoc2, then you will want to replace
  `https://app.gsoc2.com` with your own domain.
</Note>
...
In the **Set up Single Sign-On with SAML** screen, copy the **Login URL** and **SAML Certificate** to use when finishing configuring Azure SAML in Gsoc2.

![Azure SAML identity provider values 1](../../../images/sso/azure/idp-values.png)

In the **Properties** screen, copy the **Application ID** to use when finishing configuring Azure SAML in Gsoc2.

![Azure SAML identity provider values 2](../../../images/sso/azure/idp-values-2.png)

Back in Gsoc2, set **Login URL**, **Azure Application ID**, and **SAML Certificate** from above. Once you've done that, press **Update** to complete the required configuration.

![Azure SAML paste identity provider values](../../../images/sso/azure/idp-values-3.png)

<Note>
When pasting the certificate into Gsoc2, you'll want to retain `-----BEGIN
...
8. Return to Gsoc2 and enable SAML SSO.

Enabling SAML SSO enforces all members in your organization to only be able to log into Gsoc2 via Azure.

![Azure SAML assignment](../../../images/sso/azure/enable-saml.png)

<Note>
   If you're configuring SAML SSO on a self-hosted instance of Gsoc2, make sure to
