...
func CheckForUpdate() {
	if checkEnv := os.Getenv("GSOC2_DISABLE_UPDATE_CHECK"); checkEnv != "" {
		return
	}
	latestVersion, publishedDate, err := getLatestTag("Gsoc2", "gsoc2")
	if err != nil {
		log.Debug().Err(err)
	...
	if latestVersion != CLI_VERSION && daysSinceRelease > 2 {
		...
		msg := fmt.Sprintf("%s %s %s %s",
			yellow("A new release of gsoc2 is available:"),
			blue(CLI_VERSION),
			black("->"),
			blue(latestVersion),
		)
...
func GetUpdateInstructions() string {
	...
	switch os {
	case "darwin":
		return "To update, run: brew update && brew upgrade gsoc2"
	case "windows":
		return "To update, run: scoop update gsoc2"
	case "linux":
		pkgManager := getLinuxPackageManager()
		switch pkgManager {
		case "apt-get":
			return "To update, run: sudo apt-get update && sudo apt-get install gsoc2"
		case "yum":
			return "To update, run: sudo yum update gsoc2"
		case "apk":
			return "To update, run: sudo apk update && sudo apk upgrade gsoc2"
		case "yay":
			return "To update, run: yay -Syu gsoc2"
		default:
			return ""
		}
	default:
