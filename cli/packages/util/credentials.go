...
func GetUserCredsFromKeyRing(userEmail string) (credentials models.UserCredentials, err error) {
	...
	if err != nil {
		if err == keyring.ErrUnsupportedPlatform {
			return models.UserCredentials{}, errors.New("your OS does not support keyring. Consider using a service token https://gsoc2.com/docs/documentation/platform/token")
	...
	if ConfigFileExists() {
		...
		if err != nil {
			if strings.Contains(err.Error(), "credentials not found in system keyring") {
				return LoggedInUserDetails{}, errors.New("we couldn't find your logged in details, try running [gsoc2 login] then try again")
			} else {
		...
		config.GSOC2_URL_MANUAL_OVERRIDE = config.GSOC2_URL
		//configFile.LoggedInUserDomain
		//if not empty set as gsoc2 url
		if configFile.LoggedInUserDomain != "" {
			config.GSOC2_URL = configFile.LoggedInUserDomain
		}
