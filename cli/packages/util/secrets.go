...
func GetPlainTextSecretsViaJTW(JTWToken string, receiversPrivateKey string, workspaceId string, environmentName string, tagSlugs string, secretsPath string, includeImports bool) ([]models.SingleEnvironmentVariable, error) {
	...
	if len(currentUsersPrivateKey) == 0 || len(encryptedWorkspaceKeySenderPublicKey) == 0 {
		...
		PrintErrorMessageAndExit("Some required user credentials are missing to generate your [plainTextEncryptionKey]. Please run [gsoc2 login] then try again")
	}
...
func GetAllEnvironmentVariables(params models.GetAllSecretsParameters) ([]models.SingleEnvironmentVariable, error) {
	var gsoc2Token string
	if params.Gsoc2Token == "" {
		gsoc2Token = os.Getenv(GSOC2_TOKEN_NAME)
	} else {
		gsoc2Token = params.Gsoc2Token
	}
	...
	if gsoc2Token == "" {
		...
		secretsToReturn, _, errorToReturn = GetPlainTextSecretsViaServiceToken(gsoc2Token, params.Environment, params.SecretsPath, params.IncludeImport)
	}
...
func ExpandSecrets(secrets []models.SingleEnvironmentVariable, gsoc2Token string) []models.SingleEnvironmentVariable {
	...
	for i, sec := range secrets {
		...
		expandedVal := recursivelyExpandSecret(expandedSecs, interpolatedSecs, func(env string, secPaths []string, secKey string) string {
			...
			if crossRefSec, ok := crossEnvRefSecs[uniqKey]; !ok {
				...
				refSecs, err := GetAllEnvironmentVariables(models.GetAllSecretsParameters{Environment: env, Gsoc2Token: gsoc2Token, SecretsPath: secPath})
				if err != nil {
