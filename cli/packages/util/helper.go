...
func RequireLogin() {
	...
	if configFile.LoggedInUserEmail == "" {
		PrintErrorMessageAndExit("You must be logged in to run this command. To login, run [gsoc2 login]")
	}
}

func RequireServiceToken() {
	serviceToken := os.Getenv(GSOC2_TOKEN_NAME)
	if serviceToken == "" {
...
func RequireLocalWorkspaceFile() {
	...
	if workspaceFilePath == "" {
		PrintErrorMessageAndExit("It looks you have not yet connected this project to Gsoc2", "To do so, run [gsoc2 init] then run your command again")
	}

	workspaceFile, err := GetWorkSpaceFromFile()
	if err != nil {
		HandleError(err, "Unable to read your project configuration, please try initializing this project again.", "Run [gsoc2 init]")
	}

	if workspaceFile.WorkspaceId == "" {
		PrintErrorMessageAndExit("Your project id is missing in your local config file. Please add it or run again [gsoc2 init]")
	}
}
