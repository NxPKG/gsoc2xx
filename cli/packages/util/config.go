...
func WriteInitalConfig(userCredentials *models.UserCredentials) error {
	...
	configFile := models.ConfigFile{
		...
		LoggedInUserDomain: config.GSOC2_URL,
...
func WorkspaceConfigFileExistsInCurrentPath() bool {
	if _, err := os.Stat(GSOC2_WORKSPACE_CONFIG_FILE_NAME); err == nil {
		return true
	} else {
		log.Debug().Err(err)
		return false
...
// FindWorkspaceConfigFile searches for a .gsoc2.json file in the current directory and all parent directories.
func FindWorkspaceConfigFile() (string, error) {
	...
	for {
		path := filepath.Join(dir, GSOC2_WORKSPACE_CONFIG_FILE_NAME)
		_, err := os.Stat(path)
		if err == nil {
			// file found
	...
	// file not found
	return "", fmt.Errorf("file not found: %s", GSOC2_WORKSPACE_CONFIG_FILE_NAME)
...
// Get the gsoc2 config file and if it doesn't exist, return empty config model, otherwise raise error
