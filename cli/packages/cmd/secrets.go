...
import (
	...
	"github.com/Gsoc2/gsoc2-merge/packages/visualize"
...
var secretsSetCmd = &cobra.Command{
	...
	Run: func(cmd *cobra.Command, args []string) {
		...
		if loggedInUserDetails.LoginExpired {
			util.PrintErrorMessageAndExit("Your login session has expired, please run [gsoc2 login] and try again")
		}
		...
		if len(currentUsersPrivateKey) == 0 || len(encryptedWorkspaceKeySenderPublicKey) == 0 {
			...
			util.PrintErrorMessageAndExit("Some required user credentials are missing to generate your [plainTextEncryptionKey]. Please run [gsoc2 login] then try again")
		}
...
func getSecretsByNames(cmd *cobra.Command, args []string) {
	...
	secrets, err := util.GetAllEnvironmentVariables(models.GetAllSecretsParameters{Environment: environmentName, Gsoc2Token: gsoc2Token, TagSlugs: tagSlugs, SecretsPath: secretsPath})
	if err != nil {
...
func init() {
	secretsGenerateExampleEnvCmd.Flags().String("token", "", "Fetch secrets using the Gsoc2 Token")
	...
	secretsGetCmd.Flags().String("token", "", "Fetch secrets using the Gsoc2 Token")
	...
	getCmd.Flags().String("token", "", "Fetch folders using the gsoc2 token")
	...
	secretsCmd.Flags().String("token", "", "Fetch secrets using the Gsoc2 Token")
