...
var switchCmd = &cobra.Command{
	Use:                   "switch",
	Short:                 "Used to switch between Gsoc2 profiles",
	DisableFlagsInUseLine: true,
	Example:               "gsoc2 switch",
	Args:                  cobra.ExactArgs(0),
	...
	Run: func(cmd *cobra.Command, args []string) {
		...
		if err != nil {
			util.HandleError(err, "[gsoc2 user switch]: Unable to get logged Profiles")
		}

		//prompt user
		profile, err := LoggedInUsersPrompt(loggedInProfiles)
		if err != nil {
			util.HandleError(err, "[gsoc2 user switch]: Prompt error")
		}

		//write to config file
		configFile, err := util.GetConfigFile()
		if err != nil {
			util.HandleError(err, "[gsoc2 user switch]: Unable to get config file")
		}
		...
		if !ok {
			...
			configFile.LoggedInUsers = append(configFile.LoggedInUsers, models.LoggedInUser{
				Email:  profile,
				Domain: config.GSOC2_URL,
			})
			//set logged in user domain
			configFile.LoggedInUserDomain = config.GSOC2_URL
...
var updateCmd = &cobra.Command{
	Use:                   "update",
	Short:                 "Used to update properties of an Gsoc2 profile",
	DisableFlagsInUseLine: true,
	Example:               "gsoc2 user update",
	Args:                  cobra.ExactArgs(0),
	Run: func(cmd *cobra.Command, args []string) {
		cmd.Help()
	},
...
var domainCmd = &cobra.Command{
	Use:                   "domain",
	Short:                 "Used to update the domain of an Gsoc2 profile",
	DisableFlagsInUseLine: true,
	Example:               "gsoc2 user update domain",
	Args:                  cobra.ExactArgs(0),
	PreRun: func(cmd *cobra.Command, args []string) {
		util.RequireLogin()
	},
	Run: func(cmd *cobra.Command, args []string) {
		...
		loggedInProfiles, err := getLoggedInUsers()
		if err != nil {
			util.HandleError(err, "[gsoc2 user update domain]: Unable to get logged Profiles")
		}

		//prompt user
		profile, err := LoggedInUsersPrompt(loggedInProfiles)
		if err != nil {
			util.HandleError(err, "[gsoc2 user update domain]: Prompt error")
		}

		domain := ""
		domainQuery := true
		if config.GSOC2_URL_MANUAL_OVERRIDE != util.GSOC2_DEFAULT_API_URL {

			override, err := DomainOverridePrompt()
			if err != nil {
				util.HandleError(err, "[gsoc2 user update domain]: Domain override prompt error")
			}

			if !override {
				domainQuery = false
				domain = config.GSOC2_URL_MANUAL_OVERRIDE
			}
		...
		configFile, err := util.GetConfigFile()
		if err != nil {
			util.HandleError(err, "[gsoc2 user update domain]: Unable to get config file")
		}
...
func LoggedInUsersPrompt(profiles []string) (string, error) {
	prompt := promptui.Select{Label: "Which of your Gsoc2 profiles would you like to use",
		Items: profiles,
		Size:  7,
	}
