...
var runCmd = &cobra.Command{
	Example: `
	gsoc2 run --env=dev -- npm run dev
	gsoc2 run --command "first-command && second-command; more-commands..."
	`,
	Use:                   "run [any gsoc2 run command flags] -- [your application start command]",
	...
	Run: func(cmd *cobra.Command, args []string) {
		...
		gsoc2Token, err := cmd.Flags().GetString("token")
		if err != nil {
		...
		secrets, err := util.GetAllEnvironmentVariables(models.GetAllSecretsParameters{Environment: environmentName, Gsoc2Token: gsoc2Token, TagSlugs: tagSlugs, SecretsPath: secretsPath, IncludeImport: includeImports})
		...
		if shouldExpandSecrets {
			secrets = util.ExpandSecrets(secrets, gsoc2Token)
		}
		...
		Telemetry.CaptureEvent("cli-command:run", posthog.NewProperties().Set("secretsCount", len(secrets)).Set("environment", environmentName).Set("isUsingServiceToken", gsoc2Token != "").Set("single-command", strings.Join(args, " ")).Set("multi-command", cmd.Flag("command").Value.String()).Set("version", util.CLI_VERSION))
...
func filterReservedEnvVars(env map[string]models.SingleEnvironmentVariable) {
	for _, reservedEnvName := range reservedEnvVars {
		if _, ok := env[reservedEnvName]; ok {
			...
			util.PrintWarning(fmt.Sprintf("Gsoc2 secret named [%v] has been removed because it is a reserved secret name", reservedEnvName))
		}
	}
	...
	for _, reservedEnvPrefix := range reservedEnvVarPrefixes {
		for envName := range env {
			if strings.HasPrefix(envName, reservedEnvPrefix) {
				delete(env, envName)
				util.PrintWarning(fmt.Sprintf("Gsoc2 secret named [%v] has been removed because it contains a reserved prefix", envName))
			}
		}
	}
}
...
func init() {
	...
	runCmd.Flags().String("token", "", "Fetch secrets using the Gsoc2 Token")
...
func executeSingleCommandWithEnvs(args []string, secretsCount int, env []string) error {
	...
	color.Green("Injecting %v Gsoc2 secrets into your application process", secretsCount)
