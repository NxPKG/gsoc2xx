...
var vaultSetCmd = &cobra.Command{
	...
	Run: func(cmd *cobra.Command, args []string) {
		...
		if wantedVaultTypeName == "auto" || wantedVaultTypeName == "file" {
			...
			fmt.Printf("\nSuccessfully, switched vault backend from [%s] to [%s]. Please login in again to store your login details in the new vault with [gsoc2 login]\n", currentVaultBackend, wantedVaultTypeName)
...
var vaultCmd = &cobra.Command{
	Use:                   "vault",
	Short:                 "Used to manage where your Gsoc2 login token is saved on your machine",
	DisableFlagsInUseLine: true,
	Args:                  cobra.NoArgs,
	Run: func(cmd *cobra.Command, args []string) {
