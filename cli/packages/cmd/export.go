...
var exportCmd = &cobra.Command{
	...
	Example:               "gsoc2 export --env=prod --format=json > secrets.json",
	Args:                  cobra.NoArgs,
	Run: func(cmd *cobra.Command, args []string) {
		...
		secrets, err := util.GetAllEnvironmentVariables(models.GetAllSecretsParameters{Environment: environmentName, Gsoc2Token: gsoc2Token, TagSlugs: tagSlugs, WorkspaceId: projectId, SecretsPath: secretsPath})
		if err != nil {
		...
		if shouldExpandSecrets {
			substitutions := util.ExpandSecrets(secrets, gsoc2Token)
...
func init() {
	...
	exportCmd.Flags().String("token", "", "Fetch secrets using the Gsoc2 Token")
