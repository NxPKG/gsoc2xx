...
# example: v0.0.98
LATEST_RELEASE_VERSION=$(curl -s "https://api.github.com/repos/Gsoc2/gsoc2/releases/latest" | grep '"tag_name":' | sed -E 's/.*"([^"]+)".*/\1/')

# example: 0.0.98
NUMERIC_RELEASE_VERSION="${LATEST_RELEASE_VERSION:1}"

DOWNLOAD_LINK="https://github.com/Gsoc2/gsoc2/releases/download/${LATEST_RELEASE_VERSION}/gsoc2_${NUMERIC_RELEASE_VERSION}_${PLATFORM}_${ARCH}.tar.gz"
...
mkdir -p gsoc2_temp_download_folder

cd gsoc2_temp_download_folder

TEMP_DOWNLOAD_FOLDER=$(pwd)

# download latest cli
curl -L -o gsoc2-binary.tar.gz ${DOWNLOAD_LINK}

# open up the tar file
tar zxf gsoc2-binary.tar.gz

if [ "$PLATFORM" == "darwin" ] || [ $RUNNING_IN_DOCKER ] ; then
  if [[ -d /usr/local/bin ]]; then
    mv gsoc2 /usr/local/bin/
    echo "Gsoc2 CLI ${LATEST_RELEASE_VERSION} has been installed in /usr/local/bin."
  else
  ...
  mv gsoc2.exe $HOME/bin/
  echo "Gsoc2 CLI ${LATEST_RELEASE_VERSION} has been installed in $HOME/bin"
  echo "Please add $HOME/bin to your system PATH"
else
  sudo mv gsoc2 /usr/local/bin/
  echo "Gsoc2 CLI ${LATEST_RELEASE_VERSION} has been installed in /usr/local/bin."
fi  
