...
import (
	...
	"github.com/Gsoc2/gsoc2/k8-operator/packages/crypto"
...
func GetServiceTokenDetails(gsoc2Token string) (api.GetServiceTokenDetailsResponse, error) {
	serviceTokenParts := strings.SplitN(gsoc2Token, ".", 4)
	if len(serviceTokenParts) < 4 {
...
func ExpandSecrets(secrets []model.SingleEnvironmentVariable, gsoc2Token string) []model.SingleEnvironmentVariable {
	...
	for i, sec := range secrets {
		...
		expandedVal := recursivelyExpandSecret(expandedSecs, interpolatedSecs, func(env string, secPaths []string, secKey string) string {
			...
			if crossRefSec, ok := crossEnvRefSecs[uniqKey]; !ok {
				...
				refSecs, _, err := GetPlainTextSecretsViaServiceToken(gsoc2Token, "", env, secPath)
				if err != nil {
