...
const DEPLOYMENT_SECRET_NAME_ANNOTATION_PREFIX = "secrets.gsoc2.com/managed-secret"
const AUTO_RELOAD_DEPLOYMENT_ANNOTATION = "secrets.gsoc2.com/auto-reload" // needs to be set to true for a deployment to start auto redeploying

func (r *Gsoc2SecretReconciler) ReconcileDeploymentsWithManagedSecrets(ctx context.Context, gsoc2Secret v1alpha1.Gsoc2Secret) (int, error) {
	listOfDeployments := &v1.DeploymentList{}
	err := r.Client.List(ctx, listOfDeployments, &client.ListOptions{Namespace: gsoc2Secret.Spec.ManagedSecretReference.SecretNamespace})
	if err != nil {
		return 0, fmt.Errorf("unable to get deployments in the [namespace=%v] [err=%v]", gsoc2Secret.Spec.ManagedSecretReference.SecretNamespace, err)
	}
	...
	for _, deployment := range listOfDeployments.Items {
		if deployment.Annotations[AUTO_RELOAD_DEPLOYMENT_ANNOTATION] == "true" && r.IsDeploymentUsingManagedSecret(deployment, gsoc2Secret) {
...
func (r *Gsoc2SecretReconciler) IsDeploymentUsingManagedSecret(deployment v1.Deployment, gsoc2Secret v1alpha1.Gsoc2Secret) bool {
	managedSecretName := gsoc2Secret.Spec.ManagedSecretReference.SecretName
...
func (r *Gsoc2SecretReconciler) ReconcileDeployment(ctx context.Context, deployment v1.Deployment, secret corev1.Secret) error {
